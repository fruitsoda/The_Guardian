...
import web.checkSingleton as cs
import makeVid
import glob
import logging
import math
...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix='', tidl_load=False, kpt_label=False, nkpt=-1):
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            cs.addTagStream(str(s).split("/")[-1])
            #print("fps 수!!!!!!!!!!!!!!!!!", int(self.fps))
            thread = Thread(target=self.update, args=([i, cap, s, int(self.fps),w,h]), daemon=True)
    ...
    def update(self, index, cap, s, fps, w,h):
        ...
        while cap.isOpened():
            ...
            if len(lst)>fps*10+1:
                lst.pop(0)
            if cs.getCheckClass(str(name)) is not None:
                if cs.getCheckClass(str(name)) == 0 and cs.getTagStream(str(name)):
                    # print(cs.getCheckClass(str(name)))
                    makeVid.set_mp4(lst, str(name), fps, w, h, cs.getCountClass(str(name)))
                    #cs.setIsSend(1)
                    cs.setTagStream(str(name), False)
                    cs.setCheckClass(str(name),1,0)

            if not os.path.exists("./web/video/"+str(name)+'.mp4'):
                cs.setTagStream(str(name),True)
            #print(s,"의 lst!!!!:", lst)
            # print("fps개수",fps)
            # if len(lst) == fps*3:
            #     print(s,"write성공!!!")
...
# constructs an array that looks like [0, 2, 1, 4, 3, ... 16, 15, ...]
flip = lambda nkpt: [0] + [e for i in range(1, (nkpt+1) // 2) for e in [2*i, 2*i-1]] + ([nkpt-1] if nkpt % 2 == 0 else [])

class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix='',square=False, tidl_load=False, kpt_label=True, nkpt=-1):
        self.img_size = img_size
        ...
        assert len(self.flip_index) == nkpt
        try:
            f = []  # image files
            ...
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        ...
                        if kpt_label:
                            assert l.shape[1] == 5 + 3*self.nkpt, f'labels require {5 + self.nkpt * 3} columns each'
                            ...
                            l = kpts
                            assert l.shape[1] == 5 + 2*self.nkpt, f'labels require {5 + self.nkpt * 2} columns each after removing occlusion paramater'
                        else:
                        ...
                        ne += 1  # label empty
                        l = np.zeros((0, 5 + 2*self.nkpt), dtype=np.float32) if kpt_label else np.zeros((0, 5), dtype=np.float32)
