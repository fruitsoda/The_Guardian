    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            if webcam:  # batch_size >= 1
                ...
                #print("path[i]!!!!!!:", path[i]) # demo1.mp4, demo2.mp4
                #print("i값!!!!:", i)
            else:
                p, s, im0, frame = path, '', im0s.copy(), getattr(dataset, 'frame', 0)
            vid_p = str(p).split("/")[-1]
            p = Path(p)  # to Path -> txt파일에 순서대로 넣은 웹캠 및 영상 이름, 경로값
            ...
            if len(det):
                ...
                for c in det[:, 5].unique():
                    ...
                    #checklst.append(int(c))
                    # print("n!!!!!!!!!!!!:", int(c))
                    #int(c) -> 0,1,2 -> walking, crouch, lying
                    if int(c) == 2:
                        cs.setCheckClass(str(vid_p), 0, len(list(det[:,5])))
                    else:
                        cs.setCheckClass(str(vid_p), 1, len(list(det[:,5])))
                #print("countDetect!!!!!!:",int(countDetect))
                # Write results
                for det_index, (*xyxy, conf, cls) in enumerate(reversed(det[:,:6])):
                    ...
                    if save_img or opt.save_crop or view_img:  # Add bbox to image
                        ...
                        label = None if opt.hide_labels else (f'{names[c]}{""}' if opt.hide_conf else f'{names[c]} {conf:.2f}')
            ...
            #test_vid_writer = cv2.VideoWriter(save_path2, cv2.VideoWriter_fourcc(*'X264'),fps,(w,h))
            #print("detect.py에 있는 p이름!!",p)
            
            # Stream results
            if view_img:
                #test_vid_writer.write(im0)
